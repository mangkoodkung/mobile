...
if (typeof window.VoiceMessageHandler === 'undefined') {
  class VoiceMessageHandler {
    ...
    bindEvents() {
      ...
      document.addEventListener('click', event => {
        ...
        if (target && target.closest) {
          const voiceContent = target.closest(
            '.message-received[title="语音"] .message-content, .message-sent[title="语音"] .message-content',
          );
          if (voiceContent) {
    ...
    getReasonableDuration(messageContent) {
      ...
      console.log(
        `[Voice Message] 文字"${textContent}"(${charCount}字) -> 时长${minutes}:${seconds.toString().padStart(2, '0')}`,
      );
    ...
    showVoiceTextWithStreaming(messageText) {
      ...
      const originalText = messageText.textContent || messageText.innerText || 'อยู่นะ';
        ...
        mutation.addedNodes.forEach(node => {
          if (node.nodeType === Node.ELEMENT_NODE) {
            ...
            const voiceMessages =
              ...
              element.querySelectorAll('.message-received[title="语音"], .message-sent[title="语音"]');
